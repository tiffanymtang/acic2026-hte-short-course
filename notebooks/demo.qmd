...
```{r setup}
set.seed(331)
here::i_am(file.path("notebooks", "demo.qmd"))
...
- We first generate $n = 500$ samples and $p = 10$ covariates from a standard normal distribution $$X \sim \mathcal{N}(0, I_p)$$
...
- We specify the true heterogeneous treatment effect function as $$\tau(X) = 2 \cdot \mathbb{1}(X_1 > 0) - \mathbb{1}(X_2 > -0.5)$$
  ...
  \end{cases}
  \end{align*}
- Finally, we generate the outcome variable Y as $$Y = Z \cdot \tau(X) + \varepsilon,$$ where $\varepsilon \sim \mathcal{N}(0, \sigma^2)$ is an independent noise term with standard deviation $\sigma = 0.1$
...
```{r}
#| code-fold: true
#| fig-label: "hte-distribution"
#| fig-cap: "Distribution of the estimated heterogeneous treatment effects from different models. The vertical dashed line indicates the estimated average treatment effect (ATE)."
#| fig-height: 3
...
```{r}
#| code-fold: true
#| fig-label: "hte-paired-scatter"
#| fig-cap: "Paired scatter plot of the estimated heterogeneous treatment effects from different models. "
...
If the coefficient corresponding to `differential.prediction` is close to 1, then the model's predicted heterogeneous treatment effects are well calibrated. If the coefficient is very different from 1, then the model's predicted heterogeneous treatment effects are likely not well calibrated.
